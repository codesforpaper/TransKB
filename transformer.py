...
class Transformer(object):
    ...
    """
    def __init__(self, config, pre_trained):
        ...
        self.embeddedPosition = tf.placeholder(tf.float32, [None, config.sequenceLength, config.embedding_dim], name="embeddedPosition")

        self.config = config

        # 定义l2损失, sequenceLength, numBlocks, filters, embeddingSize, epsilon, l2RegLambda
        ...
        #                 [config.filters, config.embedding_dim + config.sequenceLength])

        #     outputs = tf.reshape(self.embeddedWords, [-1, config.sequenceLength * (config.embedding_dim + config.sequenceLength)])
        ...
        # self.embeddedWords = self.embedded + self.positionEmbedded
        # self.embeddedWords = self.embedded + self.embeddedPosition
        self.embeddedWords = self.embedded
            ...
            for i in range(config.num_blocks):
                with tf.name_scope("transformer-{}".format(i + 1)):
                    ...
                    multiHeadAtt = self._multiheadAttention(rawKeys=self.embedded, queries=self.embeddedWords,
                                                            keys=self.embeddedWords)
                    # 维度[batch_size, sequence_length, embedding_size]
                    self.embeddedWords = self._feedForward(multiHeadAtt, [config.filters, config.embedding_dim])

            # outputs = tf.layers.dense(self.embeddedWords, config.embedding_dim/2, activation=tf.nn.relu)   
            # print(outputs.get_shape(),self.embeddedWords.get_shape())         
            outputs = tf.reshape(self.embeddedWords, [-1, config.sequenceLength * (config.embedding_dim)])
            # outputs = tf.reshape(outputs, [-1, config.sequenceLength * (50)])
        ...
        with tf.name_scope("loss"):
            ...
            # self.loss = tf.reduce_mean(losses) + config.l2_reg_lambda * l2Loss
            # losses = tf.nn.sigmoid(self.predictions * self.inputY)
            losses = tf.nn.softplus(self.predictions * self.inputY)
            self.loss = tf.reduce_mean(losses) + config.l2_reg_lambda * l2Loss
    ...
    def _multiheadAttention(self, rawKeys, queries, keys, numUnits=None, causality=False, scope="multiheadAttention"):
        ...
        weights = tf.nn.softmax(scaledSimilary)
